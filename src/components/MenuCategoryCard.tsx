...
export function MenuCategoryCard({ category }: MenuCategoryCardProps) {
  return (
    <article id={category.id} className="scroll-mt-28 overflow-hidden rounded-lg border border-white/10 bg-[#101211]">
      <div className="border-b border-[#a42425]/50 bg-[#7d1b1c] px-5 py-4">
        ...
        {category.items.length ? (
          <ul className="space-y-3">
            {category.items.map((item) => (
              <li key={item.id} className="flex items-start justify-between gap-4 border-b border-white/8 pb-3 last:border-0 last:pb-0">
                <div>
                  <p className="text-sm font-medium leading-6 text-[#f4f0e8]">{item.name}</p>
                  {item.description ? (
